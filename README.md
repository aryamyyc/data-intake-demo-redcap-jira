...
Instead of requests arriving through emails (which can be missed, duplicated, or lack context), requests are submitted through a structured REDCap form and are automatically created or updated as Jira issues.
...
- Updates existing Jira issues when REDCap records change
- Uses hash‑based change detection to avoid unnecessary updates
- Runs continuously as a Dockerized service
...
1. A Docker container runs a polling process at a fixed interval (every 60 seconds)
2. All REDCap records are fetched via REDCap API
...
7. Record state is stored locally to prevent duplicates and unnecessary updates
